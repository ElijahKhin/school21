...
/*   Updated: 2022/06/26 16:34:14 by fhiedi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_uploading(t_map_data *map_data)
{
	...
	if (fd == -1)
	{
		ft_printf("ERROR: fd = -1, Wrong Map Name!");
		exit(1);
	}
	while (line)
	{
		...
		if (line == NULL || line[0] == '\n')
			break ;
		map_data->map_in_one_line = gnl_strjoin(map_data->map_in_one_line, line);
		map_data->height++;
		free(line);
	}
	free(line);
